...
import Lumen.Tools.Guardrails (Action (..), validateAction, isSafePath, isSystemPath)
import Test.Tasty (TestTree)
import Test.Tasty.Hedgehog (testProperty)

properties :: [TestTree]
properties =
  [ testProperty "P0: ReadFile with safe path is allowed"                  prop_readFile_safePath_allowed
  , testProperty "P1: WriteFile with safe path is allowed"                 prop_writeFile_safePath_allowed
  , testProperty "P2: DeleteFile is always blocked"                        prop_deleteFile_always_blocked
  , testProperty "P3: ExecuteCommand is always allowed"                    prop_executeCommand_always_allowed
  , testProperty "P4: System paths are blocked when allowSystemPaths=False" prop_systemPath_blocked
  , testProperty "P5: System paths are allowed when allowSystemPaths=True"  prop_systemPath_allowed_when_configured
  , testProperty "P6: Blocked paths are denied"                            prop_blockedPaths_denied
  , testProperty "P7: Path traversal is blocked"                           prop_pathTraversal_blocked
  , testProperty "P8: isSystemPath recognises known system directories"    prop_isSystemPath_known_dirs
  , testProperty "P9: isSystemPath rejects normal paths"                   prop_isSystemPath_rejects_normal
  ]

permissiveConfig :: SafetyConfig
permissiveConfig = SafetyConfig { allowedPaths = [], blockedPaths = [], allowSystemPaths = True }

restrictiveConfig :: SafetyConfig
restrictiveConfig = SafetyConfig { allowedPaths = [], blockedPaths = [], allowSystemPaths = False }

genSafePath :: Gen FilePath
genSafePath = do
  segments <- Gen.list (Range.linear 1 5) (Gen.string (Range.linear 1 20) Gen.alphaNum)
...
prop_deleteFile_always_blocked = property $ do
  path <- forAll genSafePath
  case validateAction (DeleteFile path) permissiveConfig of
    Blocked _ -> success
    Allowed   -> annotate "DeleteFile should never be Allowed" >> failure
...
prop_systemPath_blocked = property $ do
  sysDir <- forAll $ Gen.element ["/etc","/bin","/usr","/var","/sys","/boot","/sbin","/proc","/dev"]
  file   <- forAll $ Gen.string (Range.linear 1 20) Gen.alphaNum
  case validateAction (ReadFile (sysDir </> file)) restrictiveConfig of
    Blocked _ -> success
    Allowed   -> annotate ("System path should be blocked: " <> sysDir </> file) >> failure
...
prop_systemPath_allowed_when_configured = property $ do
  ...
  file   <- forAll $ Gen.string (Range.linear 1 20) Gen.alphaNum
  validateAction (ReadFile (sysDir </> file)) permissiveConfig === Allowed
...
prop_blockedPaths_denied = property $ do
  ...
  case validateAction (ReadFile path) config of
    Blocked _ -> success
    Allowed   -> annotate ("Blocked path should be denied: " <> path) >> failure
...
prop_isSystemPath_rejects_normal = property $ do
  ...
  assert $ not (isSystemPath ("/home/user/" <> name))
